...
`search_code`, `graph_expand`, and `central_symbols` are the **discovery
tier**: use `search_code` when you know roughly what you're looking for
("where do we parse connection strings"), `central_symbols` to orient
yourself in an unfamiliar codebase, and `graph_expand` to chase a
symbol up or down its dependency edges (who calls X, what types does X
use). All return CST byte ranges so results can flow directly into the
transform tools without re-parsing.
