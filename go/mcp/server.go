...
Complements the discovery tier: find_by_concept matches curated synonym sets against the full symbol body, where search_code does lexical FTS over name/signature/doc and semantic_search does embedding similarity. Reach for it when grep would be a 3-round walk: "find swipe handling", "where do we retry on transient failures", "anything auth-related". For exact symbol lookup use find_symbol; for call sites use find_references.`),
			mcpgo.WithString("query",
				mcpgo.Required(),
...
Returns JSON: {merged: string, conflicts: [...], stats: {...}, clean: bool}.`),
			...
			mcpgo.WithString("language",
				mcpgo.Description("Language adapter override (py, go, rs, ts, tsx, cpp, java, cs, js, rb, php, kt, swift, c, ...). When omitted, the adapter is inferred from path or ours_path."),
			),
			mcpgo.WithString("path",
